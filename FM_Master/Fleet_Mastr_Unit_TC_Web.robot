...
Messages
    ...
    Log Out from Fleet Mastr
    Comment    Go to    ${Messagespage}
    Comment    Sleep    5
    Comment    Selenium2Library.Click Element    //*[@id="messages-page"]/div[1]/ul/li[2]/a
    Comment    Sleep    3
    Comment    Selenium2Library.Click Element    //a[@class="btn btn-xs grey-gallery tras_btn"]
    Comment    Selenium2Library.Input Text    xpath=.//input[@placeholder='Enter new template name'][@type='text']    1 Test Standard
    Comment    Selenium2Library.Click Element    //*[@id="messageOpenChoiceEditor_ifr"]
    Comment    Press Key    //*[@id="messageOpenChoiceEditor_ifr"]    \\13
    Comment    sleep    2
    Comment    send    Test Message
    Comment    sleep    2
    Comment    Selenium2Library.Click Element    xpath=(.//button[contains(., 'Add')][@type='button'])[last()]
    Comment    sleep    4
    Comment    Go to    ${Messagespage}
    Comment    Sleep    5
    Comment    Execute Javascript    window.scroll(0,0);
    Comment    sleep    1
    Comment    Selenium2Library.Click Element    //*[@id="messages-page"]/div[1]/ul/li[4]/a
    Comment    Sleep    3
    Comment    Page Should Contain    1 Test Standard
    Comment    Sleep    3
...
Region Report Download
    go to    https://uat-firstmile.fleetmastr.com/reports
    sleep    5
...
Search results using the “Regions” value on Vehicle checks page
    Go To    http://uat-firstmile.fleetmastr.com/checks
    Sleep    3
...
Edit user record on User management page
    ...
    Sleep    2
    Selenium2Library.Input Text    ${UMSearchBynameTB}    HXJHX
    Comment    Selenium2Library.Input Text    ${UMSearchBynameTB}    ${last}
    Selenium2Library.Element Should Be Visible    ${UMUserEditBTN}
    Selenium2Library.Click Element    ${UMUserEditBTN}
    Sleep    5
    Comment    Selenium2Library.Element Should Be Visible    ${UMEditModel}
    Comment    Sleep    2
    Comment    send    {TAB 5}
    Comment    Sleep    2
    Comment    Selenium2Library.Click Element    first_name
    Comment    send    Harsh1
    Sleep    2
    Selenium2Library.Input Text    //input[@aria-describedby='first_name-error']    Harsh1
    Comment    Sleep    2
    send    {TAB}
    Sleep    2
    send    Shah1
    Comment    Selenium2Library.Input Text    ${UMEditLastnameTB}    Shah1
    Sleep    2
    send    {TAB 5}
    Sleep    2
    send    QA1
    Comment    Selenium2Library.Input Text    ${UMEditJobTitleTB}    QA1
    Comment    Selenium2Library.Click Element    xpath=/html/body/div[9]/div/form/div[2]/div/ul/li[2]/a
    Sleep    1
    Execute Javascript    window.scroll(0,500)
    Sleep    1
    Selenium2Library.Click Element    xpath=xpath:(.//button[contains(normalize-space(@class), "btn red-rubine btn-padding submit-button col-md-6")])[last()]
    Sleep    3
    Page Should Contain    Data has been saved successfully.
    Sleep    5
...
Delete user record on User management page
    ...
    Sleep    2
    Selenium2Library.Input Text    ${UMSearchBTN}    ${last}
